...
class OpenQASMParser(Lark):

    DEFAULT_INCLUDE_PATH = [
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "includes")
    ]

    INCLUDE_REGEX = re.compile(r"include\s+\"([^\"]+)\"\s*;(?:\s*//.*)?")

    def __init__(self, grammar, include_paths: typing.Iterable[str] = None, **options):
        super().__init__(grammar, **options)
        self._include_paths = (
            list(include_paths or []) + OpenQASMParser.DEFAULT_INCLUDE_PATH
        )

    @classmethod
    def open(cls, grammar_filename, rel_to=None, **options) -> "OpenQASMParser":
        if rel_to:
    ...
    def _replace_includes(self, text: str) -> str:
        ...
        match = re.search(OpenQASMParser.INCLUDE_REGEX, text)
        while match:
...
parser = OpenQASMParser.open(
    os.path.join(here, "grammars", "openqasm.lark"), parser="lalr"
)
